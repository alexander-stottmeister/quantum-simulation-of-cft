...
is a central open problem in theoretical physics, with unique challenges ranging from the infinite number of degrees of freedom to the possibility of large quantum entanglement \cite{douglasFoundationsQuantumField2012,seibergNathanSeiberg20152014,wittenAPSMedalExceptional2018}.
...
becomes possible, with a slew of recent realizations \cite{smithSimulatingQuantumManybody2019,zhangObservationManybodyDynamical2017,bernienProbingManybodyDynamics2017},
...
\cite{durrInitioDeterminationLight2008,creutzQuarksGluonsLattices1985}. However, effective field theory does not provide us with all the answers. In particular, \emph{conformal field theories} with scaling symmetries mixing the infrared and ultraviolet necessitate an understanding of the \emph{continuum limit}. Conformal field theory (CFT) \cite{belavinInfiniteConformalSymmetry1984,francescoConformalFieldTheory1997} epitomizes the outstanding challenges the development of quantum simulation algorithms faces \cite{preskillSimulatingQuantumField2018,ziniConformalFieldTheories2018}. In particular CFTs are massless and, in $1\!+\!1$ dimensions, have an infinite-dimensional group of local conformal symmetries. Thus, the simulation of their dynamics and continuous scaling symmetries via discretized approximations is particularly problematic as one needs to understand or construct the continuum limit to assess its accuracy. Fortunately, we are on the cusp of a satisfactory mathematical formulation of CFT, providing us with a unique opportunity to compare discrete approximations directly with rigorous continuum results \cite{fredenhagenSuperselectionSectorsBraid1992,borcherdsVertexAlgebrasKacMoody1986,lepowskyIntroductionVertexOperator2004,segalDefinitionConformalField2004,ziniConformalFieldTheories2018,stottmeisterOperatoralgebraicRenormalizationWavelets2020,honglerConformalFieldTheory2019}. Further, there is a rich supply of physically relevant examples to prototype quantum simulation algorithms, ranging in applications from phase transitions to quantum gravity \cite{cardyScalingRenormalizationStatistical1996,maldacenaLargeLimitSuperconformal1998}.

We meet the above challenges \cite{preskillSimulatingQuantumField2018,ziniConformalFieldTheories2018} and introduce a new approach to the quantum simulation of CFT dynamics. We overcome, by appealing to the recently developed \emph{operator-algebraic renormalization} (OAR) \cite{osborneContinuumLimitsQuantum2019,stottmeisterOperatoralgebraicRenormalizationWavelets2020,morinelliScalingLimitsLattice2020}, the twin difficulties presented by the comparison with the continuum limit and the implementation of local scaling transformations within a lattice formulation. We introduce a variety of techniques, including \emph{wavelets} \cite{daubechiesTenLecturesWavelets1992}, to rigorously construct the continuum limit and provide a quantum algorithm to coherently simulate the action of local conformal transformations via a remarkable formula due to Koo and Saleur \cite{kooRepresentationsVirasoroAlgebra1994b}. By simulating the entire physically relevant Hilbert space at a given spatial scale we are not only able to efficiently calculate four-point and higher correlators, but also more complicated observables. The complexity of the quantum algorithms we present here scales as \rev{$\delta^{-3/2}$} with the accuracy $\delta$ required\rev{---a Heisenberg-limited measurement cost $\delta^{-1}$ from phase estimation together with a discretization cost $\delta^{-1/2}$ set by the $1/n^{2}$ convergence of the lattice approximation} \footnote{Note: the algorithms also scale with the required localization of the observables and the order of the generator of local conformal transformations.}. Remarkably, we find that the resulting quantum algorithm can provide promising approximations on near-term quantum hardware with as few as \rev{$128$ lattice sites---i.e.\ $256$ logical qubits at spatial scale $N\!=\!6$} (see Supplemental Material). 

The quantum algorithm presented here may be compared with efficient classical algorithms based on tensor networks. Very recent work has led to dramatic progress in the classical simulation of CFTs, in particular, for the computation of equal-time vacuum correlation functions, including both rigorous approximations and numerical procedures \cite{milstedExtractionConformalData2017a,zouConformalFieldsOperator2020,zouEmergenceConformalSymmetry2020,zouConformalDataRenormalization2018a,konigMatrixProductApproximations2017,konigMatrixProductApproximations2016,haegemanRigorousFreeFermionEntanglement2018,singhHolographicConstructionQuantum2016}. However, a core obstruction in the classical setting remains the simulation of quantum dynamics and conformal symmetries: these processes easily create massive entanglement, obstructing their simulation via tensor networks which scale, in the worst case, exponentially with the number of lattice sites. 

\paragraph*{Representing conformal fields on a quantum computer.}
We work in the Hamiltonian formulation of conformal field theory and with natural units \cite{francescoConformalFieldTheory1997}. We simplify the exposition using the example of relativistic Dirac fermions on the circle with circumference $2L$. Periodic or antiperiodic boundary conditions realize either the \emph{Ramond} or \emph{Neveu-Schwarz} sector. The real chiral components of the massless theory give rise to CFTs with central charge $c\!=\!\frac12$ \cite{francescoConformalFieldTheory1997}. To simulate a CFT on a quantum computer we use a \emph{staggered-lattice approximation} \cite{SusskindLatticeFermions} $\Lambda_{N}\!=\!\varepsilon_{N}\{-L_{N},..., L_{N}-1\}\!\subset\!\varepsilon_{N}  \mathbb{Z}$, in finite volume ($\varepsilon_NL_{N}\!=\!L$). 
This results in the Hamiltonian \footnote{Although this particular lattice model is exactly solvable via a Fourier transform followed by a Bogoliubov transformation the existence of, and simulation of, the continuum limit is still extremely nontrivial; see, e.g., \cite{ziniConformalFieldTheories2018}\rev{, whose low-energy scaling limit is dual to OAR: Zini and Wang fix the Hilbert-space sector (via connecting unitaries on energy-bounded subspaces) and renormalize the observables as sesquilinear forms, whereas OAR fixes the observable algebra and renormalizes the states. Taking the GNS representations of the renormalized states at each scale turns the OAR scaling maps into intertwining isometries and thereby recovers their compatibility condition $\phi_N O_N\!=\!\tau_N(O_N)\phi_N$, with the spatial scale $N$ playing the role of their energy cutoff; a detailed comparison is given in \cite{osborneConformal2021}, Sec.~1.2}.}.
\begin{widetext}
\begin{align}
\label{eq:stagH}
...
\end{align}
where $B_{N}$ refers to appropriate toroidal boundary conditions (either sector dependent or sector independent) \cite{SchuetzDualityTwistedBoundary}.
...
There is considerable room for optimization here, more sophisticated representations will deliver improved resource requirements \cite{jordanUberPaulischeAquivalenzverbot1928,bravyiFermionicQuantumComputation2002,ballFermionsFermionFields2005,verstraeteMappingLocalHamiltonians2005,cadeStrategiesSolvingFermiHubbard2020}. 

\paragraph*{Description of the quantum simulation algorithm.}
The simulation algorithm consists of four steps, described and analyzed in the sequel \rev{(a detailed resource accounting and the explicit ground-state-preparation circuit are given in the Supplemental Material)}.
\begin{enumerate}
  ...
  \item Evolve the system according to a discretized local conformal symmetry transformation, via the Suzuki-Trotter decomposition.  
...
\end{align*}
where $|\Omega\rangle$ is the CFT state (not necessarily the vacuum), $A$ and $B$ are fields/observables, local in either real or momentum space, the subscript $t$ denotes Heisenberg-type evolution according to the dynamics generated by a local conformal transformation, e.g., $B_t\!\equiv\!e^{it L_k} B e^{-it L_k}$ ($L_k$ -- a Virasoro generator), the tildes represent discretized approximations. By increasing the number $n$ of lattice sites, errors can be reduced according to $1/n^{2}$. Implied constants depend on the order $k$ of the local conformal transformation desired. These error bounds generalize to multipoint correlation functions with $m$ operators; a simple bound shows an error growth quadratic in $m$. An important feature of our approach is that $A$ and $B$ are \emph{not} required to be bounded operators in the continuum limit, such as  the Virasoro generators. There is no mass gap determining a length scale for CFTs, so the scale for the simulation problem is determined by the desired spatial or frequency localization of the field operators appearing in a correlation function. Having chosen an accuracy $\delta$, we choose \emph{two} scales, $M\!<\!N$, which are large enough to ensure that the discretization errors incurred are less than $\delta$. The first scale $M$ represents the length scale -- the spatial or momentum \emph{localization} -- of the desired observables and $N$ represents the ultimate \emph{ultraviolet lattice cutoff} of the quantum simulation. Thus, we require $2n\!=\!2^{N+2}$ qubits to carry out a simulation of observables defined at \rev{observation scale} $M\!=\!N-1$. \rev{In the renormalization-group language below this means: the renormalized observables $\widetilde A,\widetilde B$ live on the observation algebra $\fA_{M}$ and are embedded into the simulated scale-$N$ lattice by the scaling map $\alpha^{M}_{N}$ defined below; the exact correlator is the scaling limit at fixed observation scale $M$ as the source scale $N\!\to\!\infty$ (equivalently, as the number $r\!=\!N\!-\!M$ of renormalization steps $\to\!\infty$), and $\delta$ bounds the deviation of the finite-$N$ approximant from this limit.}

\paragraph*{Discretised representations and renormalization.}
We use operator-algebraic renormalization \cite{stottmeisterOperatoralgebraicRenormalizationWavelets2020,morinelliScalingLimitsLattice2020,brothierCanonicalQuantization1dimensional2019,osborneContinuumLimitsQuantum2019}, a sophisticated new implementation of Wilsonian renormalization in the Heisenberg picture, to build the continuum limit as a sequence of discretized approximations which, in turn, supply us with the lattice models we simulate. OAR may be summarized as follows. We consider a family, $\{\fA_{N},\cH_{N},H^{(N)}_{0}\}_{N}$, of quantum lattice systems at (spatial) resolution $\vep$ with $N\!=\!-\log_2(\vep)$. At each scale $N$, we have an \emph{algebra} $\fA_{N}$ of fields/observables acting on the Hilbert space $\cH_{N}$ of the lattice, with Hamiltonian $H^{(N)}_{0}$. We compare the lattice systems at different scales, $N_1\!<\!N_2$, via a \emph{coarse-graining operation}, $\cE^{N_{2}}_{N_{1}}\!(\rho^{(N_{2})}_{0})\!=\!\rho^{(N_{1})}_{N_{2}-N_{1}}$, between states $\rho^{(N)}$ at different scales. Here $\mathcal{E}$ is interpreted in the \emph{Schr\"odinger picture}, however, one may equivalently describe the RG in the \emph{Heisenberg picture}, where the coarse-graining map, now called the \emph{scaling map}, acts dually on operators: $\alpha^{N_{1}}_{N_{2}}:\fA_{N_{1}}\rightarrow\fA_{N_{2}}$. \rev{We denote by $\rho^{(N)}_{0}$ the \emph{initial state} at scale $N$ (lattice spacing $\vep\!=\!2^{-N}$), and by $\rho^{(N)}_{r}\!=\!\cE^{N+r}_{N}(\rho^{(N+r)}_{0})$ its \emph{$r$th renormalization}: the state on $\fA_{N}$ obtained by coarse-graining an initial state from the finer scale $N\!+\!r$. The subscript $r$ counts renormalization steps---it is not a scale---while the superscript $N$ is the (fixed) scale at which one observes.} Requiring $\alpha^{N_{1}}_{N_{2}}$ to be unital and completely positive, i.e.~it preserves probability and sends states to states, entails equality of the partition functions of the initial and renormalized states.

\begin{figure}[h]
\begin{center}
\includegraphics{figure0.pdf}\\[10pt]
\rev{\includegraphics[scale=0.88]{figure8.pdf}}
%\begin{tikzpicture}
%	
...
%\end{tikzpicture}
\caption{\rev{Top:} Wilson's triangle of renormalization \cite{wilsonRenormalizationGroupCritical1975, stottmeisterOperatoralgebraicRenormalizationWavelets2020}: Vertical lines represent renormalization steps, either by coarse graining states ($\cE$'s) or by refining fields ($\alpha$'s). Horizontal lines represent sequences of renormalized states at a fixed scale. \rev{Bottom: the two scales of the simulation. Observables $A,B\in\fA_{M}$ live at the observation scale $M$ and are embedded by the scaling map $\alpha^{M}_{N}$ into the simulated lattice at the ultraviolet cutoff scale $N$ ($n=2^{N+1}$ sites, $2n$ qubits), $r=N-M$ renormalization steps deeper; states are coarse-grained back by $\cE^{N}_{M}$, and the discretization error decays as $2^{-sN}$.}}
...
A \textit{scaling limit} $\rho^{(N)}_{\infty}$ is a limit state \rev{$\lim_{r\rightarrow\infty}\rho^{(N)}_{r}\!=\!\rho^{(N)}_{\infty}$ on $\fA_N$---refining the source lattice to the continuum at the fixed observation scale $N$---}which is stable under coarse-graining \footnote{Note that we have described everything in terms of trace-class density operators, however, the considerations here may be extended directly to states on $C^*$ algebras without a density-operator representation. See \cite{osborneConformal2021} for further information.}:
\begin{align}
...
Ultimately, we are interested in the \emph{double scaling limit} $\rho_{\infty}^{(\infty)}\!\equiv\!\lim_{N\rightarrow \infty} \rho_\infty^{(N)}$, realizing the continuum limit, see Fig.~\ref{fig:statetrianglerg} (existing because of \eqref{eq:staterglimit} without additional constraints). The scaling limit of the family of Hamiltonians $H^{(N)}_{0}$ is understood via a limit, $\tau^{(\infty)}_{t}$, of discretized Heisenberg dynamics $\tau^{(N)}_{t}\!=\!e^{i t H^{(N)}_{0}}\!(\!\ \cdot\!\ )e^{-i t H^{(N)}_{0}}$. Thus, a triple $\{\fA,\rho,\tau_{t}\}$, where $\fA\!\equiv\! \fA_{\infty}$, $\rho\!\equiv\!\rho^{(\infty)}_{\infty}$, and $\tau_t\!\equiv\! \tau_t^{(\infty)}$, is called a \emph{scaling limit} of $\{\fA_{N},\cH_{N},H^{(N)}_{0}\}_{N}$.

The success of OAR depends crucially on the coarse-graining map $\mathcal{E}$, chosen to preserve the quantum coherence in low-energy degrees of freedom. Two particularly powerful procedures are realized in real and momentum space. For the real-space scheme we choose 
\begin{align}
\label{eq:waveletrg}
	\alpha_{N+1}^N(\psi_x^{(j)}) & \!=\!\sum_{l\in \mathbb{Z}} c_l \psi_{x-\vep_{N+1}l}^{(j)},
\end{align}
for the \emph{low-pass filter} $\{c_l\}_{l\in \mathbb{Z}}$ of a compactly supported orthogonal scaling function $s(x)\!=\!\sqrt{2}\!\sum_{l\in\mathbb{Z}} \!c_l s(2x\!-\!l)$ \footnote{Only finitely many of the $c_l$ are nonzero, so that the sum appearing in (\ref{eq:waveletrg}) is well defined \rev{as long as the fine lattice contains more sites than the filter length, i.e.\ $2^{N+2}\ge\#\{l:c_l\neq0\}$}.}.
%\begin{align*}
...
\end{align*}
The action of this nonunitary operator may be simulated, e.g., by introducing an ancilla qubit initialized in the state $\frac{1}{\sqrt{2}}(|0\rangle+|1\rangle\!)$, applying the unitary gate $V\!=\!U_{\text{FT}}\sigma^x_{-\frac{\pi}{L}(-\!L_{N\!+\!1}\!+\!\frac12)}... \sigma^x_{k\!-\!\frac{\pi}{L}}(\sigma_{k}^z\!\otimes\!|0\rangle\langle
...
\paragraph*{Measuring observables.} To measure a desired observable $\widehat{O}$, e.g., $\widehat{O}\!=\!\widehat{\psi}_k^{(j)}$, one may employ standard phase estimation \cite{nielsenQuantumComputationQuantum2000,childsQuantumSearchMeasurement2002}. This requires the addition of $r$ ancilla qubits to represent the readout, where $r$ is required to be large enough to resolve all of the eigenvalues of $\widehat{O}$. Repeating this procedure delivers an estimate for the expectation value
\begin{align*}
	\langle\widehat{O} \rangle & \!=\! \langle\Phi'|\widehat{O}|\Phi'\rangle\;.
\end{align*}

\paragraph*{Error analysis.} The errors in the quantum simulation arise from several sources. The first, and easiest to control, is the quantum projection noise arising in the estimation of $\langle \widehat{O}\rangle$. This may be reduced in the standard way via parallel repetition. The second salient source of error arises from the quantum simulation of the unitary $e^{-i s (L_k^{(N)}+\overline{L}_k^{(N)})}$ via, e.g., product formulas \cite{childsNearlyOptimalLattice2019}, and may be reduced by exploiting higher-order approximations. Exploiting state-of-the-art estimates, one requires quantum gates $(nt)^{1+o(1)}$ to reduce this error to below $\delta/2$. The \rev{third source, the} state preparation, is considered exact (up to the gate compilation required to implement the single- and two-qubit unitaries). The fourth source of error arises from the discretization itself. \rev{Here the error in $\langle\Omega_N|\widetilde A\,\widetilde B_t|\Omega_N\rangle-\langle\Omega|A\,B_t|\Omega\rangle$ decomposes into lattice-vacuum, field, generator, and dynamics contributions. The companion paper~\cite{osborneConformal2021}, in its revised form~\cite{osborneConformalRevised2026}, supplies the inputs: along the momentum-cutoff renormalization group a sharp one-particle rate $1/n^{2}$ for every generator order $k$, $N$-uniform Virasoro energy bounds for the lattice generators and, for the chiral Hamiltonian evolution ($k=0$), an explicit correlator-level bound with all constants; along the wavelet renormalization group the rate is $1/n^{2}$ for $k=0$ but only $1/n$ for $k\neq0$ unless the comparison is re-centred. For the local conformal transformations $k\neq0$, second quantization and lattice-vacuum convergence carry the one-particle rate to the static (vacuum, field, generator) contributions, and the unbounded Heisenberg flow on $[0,T]$ is controlled by confining it to an energy shell via the energy bounds and a Duhamel estimate, propagating the rate uniformly in $t\!\leq\!T$; the resulting correlator-level bound is of order $C_{T}/n^{2}$, growing as $O(m^{2})$ for $m$-point functions, with the prefactor not computed. The off-diagonal Hilbert--Schmidt cost vanishes for the M\"obius generators $k=0,\pm1$.} This \rev{yields}, for a given desired accuracy $\delta$ and $t\!\leq\!T$, a \rev{finite lattice size $n(\delta,T)$, uniform in $t\!\leq\!T$,} achieving this accuracy. \rev{Consistent with this $1/n^{2}$ convergence,} numerical experiments --- see the Supplemental Material --- indicate \rev{$n(\delta,T)\!\sim\!C_{T}\delta^{-1/2}$ (one-particle convergence data together with dynamical correlation functions of Virasoro generators)}, e.g., a D$10$ wavelet \rev{achieves accuracy} $\delta\!=\!1/2$ on a lattice of $128$ sites \rev{(one-particle data for $k=0$)}.

\paragraph*{Extensions.} The presented algorithm generalizes to the approximation of lattice currents of Wess-Zumino-Witten models, which we can simulate using $D$ copies of the realization exploited above\rev{, yielding the level-one current algebra $\widehat{\fu}(D)_{1}$} \cite{osborneConformal2021}. The Ising CFT may also be directly targeted using a modified approach. 

\paragraph*{Conclusions and future directions.} We have demonstrated that quantum computers may be exploited to simulate the local conformal dynamics of CFTs. The simulation algorithms are built invoking operator-algebraic renormalization realizing the continuum limit as a sequence of lattice models. Local conformal transformations are simulated via the Koo-Saleur generators. Generalizations to the Ising CFT and WZW models are directly possible \cite{osborneConformal2021}.

\begin{acknowledgments}
The authors would like to thank R.\ F.\ Werner for valuable discussions about inductive limits in quantum theory. AS would like to thank Y.\ Tanimoto for helpful discussions concerning the essential self-adjointness of smeared Virasoro generators. \rev{We thank the anonymous referees for constructive comments that prompted the expanded discussion of the relationship to other approaches, as well as the discretization-error analysis and the detailed resource accounting presented in the Supplemental Material.} This work was supported, in part, by the DFG through SFB 1227 (DQ-mat), Quantum Valley Lower Saxony, and funded by the Deutsche Forschungsgemeinschaft (DFG, German Research Foundation) under Germany's Excellence Strategy EXC-2123 QuantumFrontiers 390837967. 
\end{acknowledgments}
...
\bibitem [{Note2()}]{Note2}%
  ...
  e.g., \cite {ziniConformalFieldTheories2018}\rev{, whose low-energy scaling limit is dual to OAR: Zini and Wang fix the Hilbert-space sector (via connecting unitaries on energy-bounded subspaces) and renormalize the observables as sesquilinear forms, whereas OAR fixes the observable algebra and renormalizes the states. Taking the GNS representations of the renormalized states at each scale turns the OAR scaling maps into intertwining isometries and thereby recovers their compatibility condition $\phi_N O_N\!=\!\tau_N(O_N)\phi_N$, with the spatial scale $N$ playing the role of their energy cutoff; a detailed comparison is given in \cite{osborneConformal2021}, Sec.~1.2}.}\BibitemShut {Stop}%
...
\bibitem [{Note4()}]{Note4}%
  ...
  appearing in (\ref {eq:waveletrg}) is well defined \rev{as long as the fine
  lattice contains more sites than the filter length, i.e.\ $2^{N+2}\ge\#\{l:c_l\neq0\}$}.}\BibitemShut {Stop}%
\bibitem [{Note5()}]{Note5}%
  \BibitemOpen
...
\bibitem [{\citenamefont {Childs}\ and\ \citenamefont
  ...
  {NoStop}%
\bibitem [{\citenamefont {Osborne}\ and\ \citenamefont
  {Stottmeister}(2026)}]{osborneConformalRevised2026}%
  \BibitemOpen
  \bibfield  {author} {\bibinfo {author} {\bibfnamefont {T.~J.}\ \bibnamefont
  {Osborne}}\ and\ \bibinfo {author} {\bibfnamefont {A.}~\bibnamefont
  {Stottmeister}},\ }\bibfield  {title} {\bibinfo {title} {{Conformal field
  theory from lattice fermions: revised and expanded version (v5), with a
  critical re-read of the published version and a note on the Zini--Wang
  comparison}},\ }\bibinfo {note} {\url{https://alexander-stottmeister.github.io/lattice_cft/pdf/free_fermion_cft_v5.pdf};
  sources at \url{https://github.com/alexander-stottmeister/lattice_cft}; theorem numbers
  refer to \texttt{free\_fermion\_cft\_v5}}\ (\bibinfo
  {year} {2026})\BibitemShut {NoStop}%
\end{thebibliography}%
